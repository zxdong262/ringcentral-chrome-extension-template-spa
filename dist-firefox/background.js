...
// CONCATENATED MODULE: ./node_modules/ringcentral-embeddable-extension-common/src/spa/background.js
 ...
 * @param {object} tab
 */
var checkTab = function checkTab(tab) {
  return tab && tab.url && true; // todo adjust match rule
...
function _cb() {
  _cb = asyncToGenerator_default()(
  /*#__PURE__*/
  regenerator_default.a.mark(function _callee(tabId) {
    var tab;
...
var pageAction = chrome.pageAction || chrome.browserAction;
...
function oauth(data) {
  return new Promise(function (resolve, reject) {
    chrome.identity.launchWebAuthFlow(data, function (url) {
      var q = url.split('?')[1];
      q = parseQuery(q);
      var _q = q,
          code = _q.code,
          error = _q.error,
          error_description = _q.error_description;

      if (code) {
        resolve(code);
      } else if (error) {
        reject("".concat(error, ":").concat(error_description));
      }
    });
  });
}

function initBackground(checkTabFunc) {
...
function background_checkTab(tab) {
  return !!tab.url;
  /** url check examples
...
initBackground(background_checkTab);
